...
void lobby(ViewBuilder& ui, const TrialModel& model, std::string_view content) {
    ...
    ui.container(content, "play-workspace", ui.split(),
    ...
    ui.button("setup", "session-mods", "Session mods\n7 active · dependency set valid",
              "play:mods", "play-content", 50.0f);
    ...
    if (!solo) {
        ...
        ui.button("party", "find-games", "Friends & public games", "toast:Browser opened", "play-party");
    }
    ui.label("party", "summary", "CONTENT                 7 mods\nRULESET                 Standard\nNETWORK                 Automatic",
             80.0f, 12.0f);
    for (const char* id : {"player-one"}) ui.spec(id).focus_group = "play-party";
...
void quest(ViewBuilder& ui, const TrialModel&, std::string_view content) {
    ...
    ui.container(content, "quest-workspace", ui.split(),
                 {glayout::LengthKind::Fill, 1.0f}, {glayout::LengthKind::Fill, 1.0f}, 12.0f);
    ...
    for (const char* name : quests)
        ui.button("quest-list", std::string("quest-") + name, name + std::string("\nQuest route · 4–7 stages"),
                  std::string("select:") + name, "quest", 70.0f);
    ...
    ui.label("quest-detail", "quest-description",
             "Follow a fractured relay signal through flooded archives, fungal crossings, and a temple complex waking beneath the mountain.",
             78.0f, 14.0f);
    ui.image("quest-detail", "quest-art", "splonks-title", 132.0f);
    ui.label("quest-detail", "quest-route",
             "QUEST ROUTE\n1 North Pass   2 Mushroom Crossing   3 Flooded Archive\n4 Temple Gate   5 The Relay   6 Violet Core",
             100.0f, 14.0f);
...
void rules(ViewBuilder& ui, const TrialModel&, std::string_view content) {
    ...
    ui.label("rules-header", "rules-title", "EXPEDITION SETTINGS · 10 rules · 1 from mods", 52.0f, 16.0f);
    ui.button("rules-header", "rules-reset", "Reset defaults", "toast:Rules reset", "rules", 52.0f);
    ui.container(content, "rules-workspace", ui.split(),
                 {glayout::LengthKind::Fill, 1.0f}, {glayout::LengthKind::Fill, 1.0f}, 12.0f);
    ...
    ui.slider("rules-list", "starting-health", "Starting health", "starting-health", "rules", 1, 9, 1);
    ui.slider("rules-list", "ghost-arrival", "Ghost arrival", "ghost-arrival", "rules", 30, 300, 10);
    ...
    ui.label("rule-detail", "rule-description",
             "Lets the party enter from previously discovered quest junctions. Changes serialize into the session manifest and synchronize before launch.",
             100.0f, 14.0f);
    ui.label("rule-detail", "rule-context", "SESSION EFFECT\nActivity      Continue expedition\nAuthority     Lobby host\nSynced        Before launch",
             120.0f, 13.0f);
...
void session_mods(ViewBuilder& ui, const TrialModel&, std::string_view content) {
    ...
    ui.container(content, "session-mod-workspace", ui.split(),
                 {glayout::LengthKind::Fill, 1.0f}, {glayout::LengthKind::Fill, 1.0f}, 12.0f);
    panel(ui, "session-mod-workspace", "session-mod-list");
    ui.label("session-mod-list", "session-mod-title", "CURRENT SESSION SET · 7 PACKAGES", 34.0f, 12.0f);
    ...
    ui.label("session-mod-detail", "session-mod-info",
             "Update available. Required by Temple Weather and Pocket Expedition. Exact versions are recorded with checkpoints.",
             86.0f, 14.0f);
    ui.button("session-mod-detail", "browse-add", "Browse & add mods", "mods:browse", "session-mods");
    ui.button("session-mod-detail", "update-session-mod", "Update in this set", "toast:Update planned",
              "session-mods");
...
void build_play(ViewBuilder& ui, const TrialModel& model, std::string_view content) {
    ...
    case PlayPage::Lobby: lobby(ui, model, content); break;
    case PlayPage::Quest: quest(ui, model, content); break;
    case PlayPage::Rules: rules(ui, model, content); break;
    case PlayPage::SessionMods: session_mods(ui, model, content); break;
    }
}
