...
void bindings(ViewBuilder& ui, std::string_view content) {
    ...
    ui.container(content, "binding-workspace", ui.split(),
    ...
    constexpr const char* actions[]{"Menu Up", "Menu Down", "Menu Left", "Menu Right", "Activate",
                                     "Cancel", "Move", "Look", "Jump", "Attack", "Use", "Pause"};
    ...
    ui.label("binding-detail", "binding-methods",
             "ADD BINDING METHODS\nListen for next input · Perform an input · Browse device controls\nManual source: Right Trigger → Axis to Button · threshold 0.62 rising",
             110.0f, 13.0f);
    ui.button("binding-detail", "listen-input", "Listen for input", "binding:listen", "controls-content");
...
void devices(ViewBuilder& ui, std::string_view content) {
    ui.container(content, "control-device-workspace", ui.split(),
                 {glayout::LengthKind::Fill, 1.0f}, {glayout::LengthKind::Fill, 1.0f}, 12.0f);
    ...
    constexpr const char* devices[]{"Keyboard + Mouse", "Xbox Wireless Controller",
                                     ...
                                     "T-LCM Pedals", "Stream Deck Macro Pad"};
    ...
    ui.container("control-device-workspace", "control-device-detail", glayout::ContainerKind::Column,
...
void tuning(ViewBuilder& ui, std::string_view content) {
    ui.container(content, "tuning-workspace", ui.split(),
                 {glayout::LengthKind::Fill, 1.0f}, {glayout::LengthKind::Fill, 1.0f}, 12.0f);
    ...
    ui.slider("tuning-list", "stick-deadzone", "Stick deadzone", "stick-deadzone",
              "controls-content", 0, 1, 0.01);
    ui.slider("tuning-list", "vibration", "Vibration strength", "vibration",
              "controls-content", 0, 1, 0.01);
    ...
    ui.label("response-detail", "response-graph", "              ●\n\nRAW INPUT      0.742\nQUALIFIED      0.611",
             200.0f, 17.0f, gview::TextAlign::Center);
