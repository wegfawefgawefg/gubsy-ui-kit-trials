...
int main(int argc,char** argv){
  ...
  if(offscreen)SDL_ReleaseGPUTexture(gpu,offscreen);ImGui_ImplSDLGPU3_Shutdown();ImGui_ImplSDL3_Shutdown();ImGui::DestroyContext();SDL_ReleaseWindowFromGPUDevice(gpu,window);SDL_DestroyGPUDevice(gpu);SDL_DestroyWindow(window);SDL_Quit();return 0;
}
