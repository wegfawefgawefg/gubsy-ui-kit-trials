...
void CheckpointPicker(UiState& s) {
  const bool is_new=s.play_activity!=0;Label(is_new?"SPLONKS / PLAY / QUEST":"SPLONKS / PLAY / CHECKPOINT");Title(is_new?"Choose a quest":"Choose checkpoint");
  if(ImGui::Button("< Back to lobby",{150,36}))s.play_view=0;
  ImGui::BeginChild("checkpoint-list",{ImGui::GetContentRegionAvail().x*.48f,0},ImGuiChildFlags_Borders);
  Label(is_new?"AVAILABLE QUESTS":"THE VIOLET REACH");
  ...
  for(int i=0;i<3;i++)if(Row(is_new?quests[i]:checkpoints[i],is_new?quest_sub[i]:cp_sub[i],is_new?"SELECT":i==0?"LATEST":"BACKUP",s.selected_checkpoint==i,68))s.selected_checkpoint=i;
  ImGui::EndChild();ImGui::SameLine();
  ImGui::BeginChild("checkpoint-detail",{0,0},ImGuiChildFlags_Borders);
  Label(is_new?"QUEST ROUTE":"CHECKPOINT DETAILS");Title(is_new?quests[s.selected_checkpoint]:checkpoints[s.selected_checkpoint]);
  Row(is_new?"Start":"Stage one",is_new?"Current profile, rules, party, and mods":"The Glass Caverns");Row("Profile","Vega");Row("Recorded mod set","7 compatible packages");
  ImGui::SetCursorPosY(ImGui::GetWindowHeight()-52);
  if(PrimaryButton(is_new?"Use this quest":"Use this checkpoint",{-1,36}))s.play_view=0;
  ImGui::EndChild();
}

void SessionMods(UiState& s){
  Label("SPLONKS / PLAY / SESSION MODS");Title("Session mods");if(ImGui::Button("< Back to lobby",{150,36}))s.play_view=0;ImGui::SameLine();
  const char* tabs[]={"Current set","Browse & add"};for(int i=0;i<2;i++){if(i)ImGui::SameLine();if(ImGui::Selectable(tabs[i],s.session_mods_tab==i,0,{130,36}))s.session_mods_tab=i;}
  const char* names[]={"Base Content","Cartographer's Desk","Old Lanterns","Underground Rivers","Temple Weather","Pocket Expedition","Quiet Campfires","Mycelium Below"};const char* sub[]={"Required by game","Quest dependency","Update available","World dependency","Adds weather rules","Adds pocket quests","Cosmetic","Catalog package · ready to install"};
  ImGui::BeginChild("session-mod-list",{ImGui::GetContentRegionAvail().x*.52f,0},ImGuiChildFlags_Borders);Label(s.session_mods_tab?"ADD CONTENT TO THIS LOBBY":"CURRENT LOBBY MANIFEST");for(int i=0;i<8;i++){if(!s.session_mods_tab&&!s.session_enabled[i])continue;if(Row(names[i],sub[i],s.session_enabled[i]?"ACTIVE":"ADD",s.selected_session_mod==i,56))s.selected_session_mod=i;}ImGui::EndChild();ImGui::SameLine();
  ImGui::BeginChild("session-mod-detail",{0,0},ImGuiChildFlags_Borders);Label("SESSION PACKAGE");Title(names[s.selected_session_mod]);ImGui::TextWrapped("Changes here affect this lobby's exact content manifest. Dependencies are installed and activated together.");Row("Base Content >= 1.4.0","Required dependency","INSTALLED");Row("Lobby manifest","Recorded at launch",s.session_enabled[s.selected_session_mod]?"INCLUDED":"NOT INCLUDED");ImGui::SetCursorPosY(ImGui::GetWindowHeight()-52);bool on=s.session_enabled[s.selected_session_mod];if(PrimaryButton(on?"Remove from current session":"Install & add to current session",{-1,36}))s.session_enabled[s.selected_session_mod]=!on;ImGui::EndChild();
}

void RulesEditor(UiState& s) {
  Label("SPLONKS / PLAY / RULES");Title("Expedition settings");
  if(ImGui::Button("< Back to lobby",{150,36}))s.play_view=0;
  ImGui::BeginChild("rules-list",{ImGui::GetContentRegionAvail().x*.56f,0},ImGuiChildFlags_Borders);
  SettingRow("Expedition difficulty","Overall danger and resource pressure",[&]{const char* v[]={"Relaxed","Standard","Punishing"};ImGui::Combo("##difficulty",&s.expedition_difficulty,v,3);});
  SettingRow("Stage variation","How authored rooms are remixed",[&]{const char* v[]={"Quest-authored","Seeded remix","Fully random"};ImGui::Combo("##variation",&s.stage_variation,v,3);});
  SettingRow("Shared lives","Continues available to the party",[&]{ImGui::SliderFloat("##lives",&s.shared_lives,1,9,"%.0f");});
  SettingRow("Starting health","Hearts granted at expedition start",[&]{ImGui::SliderFloat("##health",&s.starting_health,1,8,"%.0f hearts");});
  SettingRow("Ghost arrival","Seconds before the stage begins hunting",[&]{ImGui::SliderFloat("##ghost",&s.ghost_arrival,60,300,"%.0f sec");});
  SettingRow("Shop frequency","Merchant room availability",[&]{const char* v[]={"Rare","Normal","Frequent"};ImGui::Combo("##shops",&s.shop_frequency,v,3);});
  SettingRow("Discovered shortcuts","Allow unlocked route entrances",[&]{ImGui::Checkbox("##shortcuts",&s.discovered_shortcuts);});
  SettingRow("Shared treasury","Combine party currency",[&]{ImGui::Checkbox("##treasury",&s.shared_treasury);});
  SettingRow("Friendly fire","Party attacks can damage allies",[&]{ImGui::Checkbox("##friendly",&s.friendly_fire);});
  SettingRow("Defeated players return","Co-op recovery rule",[&]{const char* v[]={"At next room","At next stage","Only at checkpoints"};ImGui::Combo("##revive",&s.revive_mode,v,3);});
  SettingRow("Enemy damage","Damage multiplier",[&]{ImGui::SliderFloat("##damage-mult",&s.enemy_damage,50,200,"%.0f%%");});
  ImGui::EndChild();ImGui::SameLine();
  ImGui::BeginChild("rules-detail",{0,0},ImGuiChildFlags_Borders);Label("SELECTED RULE");Title("Expedition rules");
  ImGui::TextWrapped("The game supplies these rule definitions. Changes are stored in the lobby and synchronized by the host before launch.");
  Row("Activity",s.play_activity==0?"Continue expedition":"New expedition");Row("Authority","Lobby host");Row("Synced","Before launch");
  ImGui::SetCursorPosY(ImGui::GetWindowHeight()-52);if(PrimaryButton("Apply and return",{-1,36}))s.play_view=0;ImGui::EndChild();
}

void Play(UiState& s) {
  if(s.play_view==1){CheckpointPicker(s);return;}
  if(s.play_view==2){RulesEditor(s);return;}
  if(s.play_view==3){SessionMods(s);return;}
  ...
  if(Row(s.play_activity==0?"Resume point":"Quest",s.play_activity==0?"Latest checkpoint · The Violet Reach":"The Violet Reach · 8 stages",s.play_activity==0?"CHOOSE CHECKPOINT >":"CHOOSE QUEST >"))s.play_view=1;
  ...
  if(Row("Expedition rules","Standard · 4 lives · ghost at 180s","EDIT ALL >"))s.play_view=2;
  if(Row("Session mods","7 active · dependency set valid","MANAGE >"))s.play_view=3;
...
void Players(UiState& s) {
  ...
  if(s.players_tab==0){Label("LOCAL PARTY");Sub("Assign devices and profiles");ImGui::Columns(2,nullptr,false);Row("P1  Moss","Profile: Moss · Xbox Wireless Controller",s.local_ready?"READY":"NOT READY",true,72);Row("P2  Open slot","Add a local player or invite a friend",nullptr,false,72);ImGui::NextColumn();Label("SELECTED PLAYER");ImGui::TextUnformatted("Moss");Row("Xbox Wireless Controller","Gamepad 0 · connected","ASSIGNED");Row("Keyboard + mouse","Shared desktop input","AVAILABLE");if(PrimaryButton("Assign device",{140,36}))s.players_tab=2;ImGui::SameLine();if(ImGui::Button(s.local_ready?"Mark not ready":"Mark ready",{140,36}))s.local_ready=!s.local_ready;ImGui::Columns(1);}
  else if(s.players_tab==1){Label("PROFILE LIBRARY");Sub("Profile data is not a save");ImGui::SameLine(ImGui::GetWindowWidth()-130);PrimaryButton("+ New profile",{110,34});ImGui::Columns(3,nullptr,false);const char* n[]={"Moss","Vega","Guest"};const char* st[]={"38h 22m · 84 runs · 21 wins","14h 11m · 31 runs · 4 wins","Never · 0 runs · 0 wins"};for(int i=0;i<3;i++){if(Row(n[i],st[i],i==s.selected_profile?"ACTIVE":"SELECT",i==s.selected_profile,120))s.selected_profile=i;if(i<2)ImGui::NextColumn();}ImGui::Columns(1);ImGui::Separator();ImGui::Text("%s's history",n[s.selected_profile]);ImGui::Columns(5,nullptr,false);const char* stats[]={"84\nPLAYS","21\nWINS","138\nDEATHS","2.4M\nSCORE","42\nREPLAYS"};for(auto* v:stats){ImGui::TextColored(Accent,"%s",v);ImGui::NextColumn();}ImGui::Columns(1);}
  else {Label("CONNECTED INPUT");Sub("Devices may belong to more than one local player");ImGui::Columns(2,nullptr,false);const char* n[]={"Xbox Wireless Controller","Keyboard + mouse","T.16000M Joystick"};const char* st[]={"Gamepad 0 · battery 82%","Desktop aggregate","14 axes · 32 buttons"};for(int i=0;i<3;i++)if(Row(n[i],st[i],i==0?"MOSS":i==1?"AVAILABLE":"UNASSIGNED",s.selected_device==i,66))s.selected_device=i;ImGui::NextColumn();Label("LIVE INPUT EXPLORER");ImGui::TextUnformatted(n[s.selected_device]);Row("Button South","idle");Row("Left X","+0.04");Row("Right Trigger","0.00");ImGui::Columns(1);}
  ImGui::EndChild();
}
...
void Settings(UiState& s) {
  ...
  else if(s.settings_tab==2){SettingRow("Subtitles","Dialogue and important callouts",[&]{ImGui::Checkbox("##sub",&s.subtitles);});SettingRow("Color distinction","Shape and label reinforcement",[&]{ImGui::Checkbox("##color",&s.color_distinction);});SettingRow("Reduced motion","Limit camera and menu animation",[&]{ImGui::Checkbox("##motion",&s.reduced_motion);});Row("Text scale","Interface and subtitle size","110%");}
  else {SettingRow("Pause when unfocused","Suspend local simulation",[&]{ImGui::Checkbox("##pause",&s.pause_unfocused);});SettingRow("Tutorial prompts","Context-sensitive hints",[&]{ImGui::Checkbox("##tutorial",&s.tutorial_prompts);});SettingRow("Damage numbers","Combat feedback",[&]{ImGui::Checkbox("##damage",&s.damage_numbers);});}
...
void Controls(UiState& s) {
  if(s.controls_view){Label("SPLONKS / CONTROLS / BINDING");Title(s.controls_view==1?"Listen for input":"Browse controls");if(ImGui::Button("< Back to bindings",{160,36}))s.controls_view=0;ImGui::BeginChild("binding-tool",{0,0},ImGuiChildFlags_Borders);if(s.controls_view==1){Label("CAPTURE ACTIVE");Title("Perform an input now");ImGui::TextWrapped("Buttons, keys, 1D axes, 2D sticks, triggers, pedals, wheels, and hats are accepted. Noise qualification is applied before binding.");Row("Xbox Wireless Controller","Waiting for qualified input...","LISTENING",true,86);if(ImGui::Button("Cancel capture",{160,36}))s.controls_view=0;}else{const char* inputs[]={"Button South (A)","Button East (B)","Left Trigger axis","Right Trigger axis","Left Stick 2D","Wheel axis","Pedal axis","Hat 0 Up"};ImGui::BeginChild("input-list",{ImGui::GetContentRegionAvail().x*.5f,0},ImGuiChildFlags_Borders);Label("DEVICE INPUTS");for(int i=0;i<8;i++)if(Row(inputs[i],i<2?"Button":"Analog / directional input",nullptr,s.browsed_input==i,48))s.browsed_input=i;ImGui::EndChild();ImGui::SameLine();ImGui::BeginChild("input-detail",{0,0},ImGuiChildFlags_Borders);Label("MANUAL BINDING");Title(inputs[s.browsed_input]);Row("Source device","Xbox Wireless Controller");Row("Transform","Raw input to selected Gubsy action");if(PrimaryButton("Use this input",{-1,36}))s.controls_view=0;ImGui::EndChild();}ImGui::EndChild();return;}
...
void Progress(UiState& s) {
  Label("SPLONKS / PROGRESS");Title("Progress");Label("GAME-PROVIDED PROGRESSION");Sub("3 campaigns · automatic checkpoints");
  const char* names[]={"The Glass Caverns","A Quiet Beginning","Old Expedition"};const char* details[]={"Moss · Temple Depths · 42%","Vega · Green Valley · 11%","Moss · Version 0.7 data"};const char* statuses[]={"12h 48m  READY","3h 06m  READY","INCOMPATIBLE"};ImGui::BeginChild("campaigns",{ImGui::GetContentRegionAvail().x*.5f,0},ImGuiChildFlags_Borders);for(int i=0;i<3;i++)if(Row(names[i],details[i],statuses[i],s.selected_progress==i,66))s.selected_progress=i;ImGui::EndChild();ImGui::SameLine();ImGui::BeginChild("progress-detail",{0,0},ImGuiChildFlags_Borders);Label("SELECTED CAMPAIGN");Title(names[s.selected_progress]);Label("ASSOCIATED PROFILE");Row(s.selected_progress==1?"VE   Vega":"MO   Moss","Ownership recorded by progression provider");Label("RECORDED MOD SET");Row("Base Content","Required package","v1.4.0");Row("Cartographer's Desk","Quest dependency","v0.8.2");Label("CHECKPOINT HISTORY");Row("Temple safe room","Today, 06:52 · Stage 4","RESUME");Row("Flooded archive","Yesterday · Stage 3","BACKUP");if(PrimaryButton("Use in Play",{160,36})&&s.selected_progress<2){s.screen=0;s.play_activity=0;}ImGui::SameLine();ImGui::Button("Export",{90,36});ImGui::EndChild();
}
...
void Mods(UiState& s) {
  ...
  const char* catalog[]={"Mycelium Below","Brassline Grapple Kit","Skybreak Caverns","Abyssal Tide","Old Lanterns","Pocket Expedition","Temple Weather","Mirror Depths","Clockwork Orchard","Lantern Cartography","Fungal Friends","Deep Relay","Cave Radio","Run History+","Accessible Traps","Quiet Ghost","Shared Wallet","Daily Seed Lab","Rope Physics","Vanilla Plus"};
  const char* installed[]={"Base Content","Cartographer's Desk","Old Lanterns","Underground Rivers","Temple Weather","Pocket Expedition","Quiet Campfires","Classic Palette"};
  const char* deps[]={"Requires Base Content","Requires Cartographer's Desk","Requires game 1.4","Requires Underground Rivers","Update available","2 dependencies","Required by 2 mods","Requires Gubsy Mod API 0.2"};
  const int count=s.mods_tab?20:8;int& selected=s.mods_tab?s.selected_mod:s.selected_installed;
  ImGui::BeginChild("mods-list",{ImGui::GetContentRegionAvail().x*.5f,0},ImGuiChildFlags_Borders);
  for(int i=0;i<count;i++){const char* name=s.mods_tab?catalog[i]:installed[i];ImGui::PushID(i);const ImVec2 start=ImGui::GetCursorScreenPos();if(ImGui::Selectable("##mod",selected==i,0,{0,62}))selected=i;auto* dl=ImGui::GetWindowDrawList();if(Assets.mod_sheet!=ImTextureID_Invalid){const float u0=float(i%5)/5.0f,u1=float(i%5+1)/5.0f;dl->AddImage(Assets.mod_sheet,{start.x+8,start.y+7},{start.x+56,start.y+55},{u0,0},{u1,1});}dl->AddText({start.x+66,start.y+9},IM_COL32(224,234,232,255),name);dl->AddText({start.x+66,start.y+33},IM_COL32(145,164,161,255),s.mods_tab?(i<8?deps[i]:"Catalog content package"):(i==2?"Update available":"Installed package"));const char* status=s.mods_tab?(i==7?"INCOMPATIBLE":i==4?"UPDATE":"93%"):i==2?"UPDATE":"INSTALLED";const ImVec2 z=ImGui::CalcTextSize(status);dl->AddText({start.x+ImGui::GetContentRegionAvail().x-z.x-10,start.y+19},ImGui::ColorConvertFloat4ToU32(i==7&&s.mods_tab?ImVec4{1,.4f,.4f,1}:Accent),status);ImGui::PopID();}
  ImGui::EndChild();ImGui::SameLine();ImGui::BeginChild("mod-detail",{0,0},ImGuiChildFlags_Borders);if(Assets.mod_sheet!=ImTextureID_Invalid){const float u0=float(selected%5)/5.0f,u1=float(selected%5+1)/5.0f;ImGui::Image(Assets.mod_sheet,{ImGui::GetContentRegionAvail().x,118},{u0,.12f},{u1,.88f});}Label(s.mods_tab?"CATALOG ENTRY":"INSTALLED PACKAGE");ImGui::TextUnformatted(s.mods_tab?catalog[selected]:installed[selected]);ImGui::TextWrapped(s.mods_tab?"Install directly into the current session or keep the package installed for later.":"Inspect, update, or remove this installed package and review its dependency relationships.");Label("COMPATIBILITY & DEPENDENCIES");Row("Base Content >= 1.4.0","Core dependency","INSTALLED");Row("Underground Rivers >= 2.2.0","Automatic dependency",s.mods_tab?"WILL INSTALL":"INSTALLED");Label("REQUIRED BY");Row("Temple Weather","Dependent package","ACTIVE");Row("Pocket Expedition","Dependent package","ACTIVE");ImGui::TextColored(selected==7&&s.mods_tab?ImVec4{1,.4f,.4f,1}:Muted,"%s",selected==7&&s.mods_tab?"This mod cannot be installed on the current API version.":"The full dependency change plan is shown before mutation.");const char* primary=s.mods_tab?(s.catalog_installed[selected]?"Installed":"Install & add to session"):"Update";if(PrimaryButton(primary,{210,36})&&s.mods_tab)s.catalog_installed[selected]=true;ImGui::SameLine();ImGui::Button(s.mods_tab?"Install only":"Open files",{110,36});ImGui::EndChild();
}
